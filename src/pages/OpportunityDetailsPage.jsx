...
export default function OpportunityDetailsPage() {
  ...
  return (
    <div>
      <Link to="/results" className="inline-flex items-center gap-1.5 text-[13px] text-ink-secondary hover:text-ink-primary transition-colors mb-6">
      ...
      <div className="p-6 lg:p-8 rounded-3xl bg-base-card ring-1 ring-base-border mb-6">
        ...
        <h1 className="text-[24px] lg:text-[28px] font-semibold tracking-tight text-ink-primary">{opportunity.title}</h1>
        <p className="text-[14.5px] text-ink-secondary mt-1.5">{opportunity.organization}</p>

        <div className="flex flex-wrap gap-x-6 gap-y-2 mt-5 text-[13px] text-ink-secondary">
          <span className="flex items-center gap-1.5">
            <MapPin size={14} className="text-ink-tertiary" /> {opportunity.location}
          </span>
          <span className="flex items-center gap-1.5">
            <Calendar size={14} className="text-ink-tertiary" /> {formatDeadline(opportunity.deadline)}
          </span>
        ...
        <div className="flex flex-wrap gap-3 mt-6">
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-accent-indigo hover:bg-accent-indigo/90 text-white text-[13.5px] font-medium transition-colors shadow-glow"
          >
          ...
          <button
            ...
            className={[
              'inline-flex items-center gap-2 px-5 py-2.5 rounded-xl text-[13.5px] font-medium ring-1 transition-colors',
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="p-6 rounded-2xl bg-base-card ring-1 ring-base-border">
            <h2 className="text-[15px] font-semibold text-ink-primary mb-4">Why this matches you</h2>
          ...
          <div className="p-6 rounded-2xl bg-base-card ring-1 ring-base-border space-y-6">
            <div>
              <h3 className="text-[13.5px] font-semibold text-ink-primary flex items-center gap-2 mb-3">
                <FileText size={15} className="text-accent-indigo" /> Eligibility
              </h3>
              {opportunity.eligibility?.length ? (
                <ul className="space-y-1.5">
                  {opportunity.eligibility.map((e) => (
                    <li key={e} className="text-[13px] text-ink-secondary flex items-start gap-2">
                      <span className="text-state-success mt-0.5">✓</span> {e}
                    </li>
                  ))}
                </ul>
              ) : (
                <p className="text-[13px] text-ink-tertiary">Not specified in the email</p>
              )}
            </div>

            <div>
              <h3 className="text-[13.5px] font-semibold text-ink-primary flex items-center gap-2 mb-3">
                <FileText size={15} className="text-accent-indigo" /> Required documents
              </h3>
              {opportunity.requiredDocuments?.length ? (
                <div className="flex flex-wrap gap-2">
                  {opportunity.requiredDocuments.map((d) => (
                    <span key={d} className="px-2.5 py-1 rounded-lg bg-base-surface ring-1 ring-base-border text-[12px] text-ink-secondary">
                      {d}
                    </span>
                  ))}
                </div>
            ...
            <div>
              <h3 className="text-[13.5px] font-semibold text-ink-primary flex items-center gap-2 mb-3">
                <Wrench size={15} className="text-accent-indigo" /> Required skills
              </h3>
              {opportunity.skills?.length ? (
                <div className="flex flex-wrap gap-2">
                  {opportunity.skills.map((s) => (
                    <span key={s} className="px-2.5 py-1 rounded-lg bg-base-surface ring-1 ring-base-border text-[12px] text-ink-secondary">
                      {s}
                    </span>
                  ))}
                </div>
            ...
            <div>
              <h3 className="text-[13.5px] font-semibold text-ink-primary flex items-center gap-2 mb-3">
                <Mail size={15} className="text-accent-indigo" /> Application information
              </h3>
              <div className="text-[13px] text-ink-secondary space-y-1">
