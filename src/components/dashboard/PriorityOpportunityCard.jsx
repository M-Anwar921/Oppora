    ...
    >
      ...
      >
        <div className="w-11 h-11 shrink-0 rounded-xl bg-base-surface ring-1 ring-base-border flex items-center justify-center">
          <span className="text-[17.2px] font-semibold text-accent-cyan tabular">#{rank}</span>
        </div>

        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 flex-wrap">
            <p className="text-[16.7px] font-semibold text-ink-primary truncate">{opportunity.title}</p>
          </div>
          <p className="text-[14.4px] text-ink-secondary mt-0.5 truncate">{opportunity.organization}</p>
        </div>

        <div className="hidden sm:flex flex-col items-end gap-1.5 shrink-0">
          <PriorityBadge priority={opportunity.priority} />
          <p className="text-[13.2px] text-ink-tertiary tabular">
            {formatDeadline(opportunity.deadline)} · {urgency.text}
          </p>
        </div>

        <div className="shrink-0 text-right">
          <p className="text-[21.8px] font-semibold tabular leading-none group-hover:text-accent-cyan transition-colors">
            {opportunity.finalScore}
          </p>
          <p className="text-[12.6px] text-ink-tertiary">/ 100</p>
        </div>
      </Link>
    </motion.div>
  )
