  ...
  return (
    <div>
      <div className="space-y-2.5 mb-4">
        {value.map((entry) => (
          ...
          >
            <div className="min-w-0">
              <p className="text-[15.5px] font-medium text-ink-primary">{entry.label}</p>
              {entry.detail && <p className="text-[14.4px] text-ink-secondary mt-0.5">{entry.detail}</p>}
            </div>
        ...
        {value.length === 0 && <p className="text-[14.9px] text-ink-tertiary">No experience added yet.</p>}
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-[1fr_1.4fr_auto] gap-2.5">
        <input
          ...
          className="px-3.5 py-2.5 rounded-lg bg-base-surface ring-1 ring-base-border focus:ring-accent-indigo/50 outline-none text-[15.5px] text-ink-primary placeholder:text-ink-tertiary"
        />
        <input
          value={detail}
        ...
        <button
          onClick={addEntry}
          className="flex items-center justify-center gap-1.5 px-4 py-2.5 rounded-lg bg-base-surface ring-1 ring-base-border hover:ring-base-borderStrong transition-shadow text-[14.9px] text-ink-secondary"
        >
          <Plus size={17} /> Add
        </button>
      </div>
