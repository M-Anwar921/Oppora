...
export default function RankLowerExplainer({ gaps }) {
  ...
  return (
    <div className="p-5 rounded-2xl bg-state-warning/5 ring-1 ring-state-warning/20">
      <p className="text-[14px] font-semibold text-ink-primary mb-4">Why this ranks lower</p>
      <div className="space-y-3">
        {gaps.map((gap) => (
          <div key={gap.label} className="flex items-start gap-3">
            ...
            >
              {gap.met ? (
                <Check size={11} className="text-state-success" strokeWidth={3} />
              ) : (
                <X size={11} className="text-state-warning" strokeWidth={3} />
              )}
            </span>
            <div className="min-w-0">
              <p className="text-[13px] font-medium text-ink-primary">{gap.label}</p>
